...
uses
  ...
  VCLTee.Chart, dspinterface, math;
...
type
  ...
  TForm1 = class(TForm)
    ...
    GroupBox1: TGroupBox;
    Chart2: TChart;
    LPFOfflineChart: TLineSeries;
    HPFOfflineChart: TLineSeries;
    DerivativeOfflineChart: TLineSeries;
    SqrOfflineChart: TLineSeries;
    MAVOfflineChart: TLineSeries;
    Chart3: TChart;
    Series7: TLineSeries;
    GroupBox2: TGroupBox;
    Chart4: TChart;
    ...
    Button7: TButton;
    FFedit: TLabeledEdit;
    OrdeMAVEdit: TLabeledEdit;
    ErrorEdit: TLabeledEdit;
    ThOfflineChart: TLineSeries;
    LPOfflineChart: TLineSeries;
    LNOfflineChart: TLineSeries;
    Button8: TButton;
    ApEdit: TLabeledEdit;
  ...
  public
    ...
    jmldataecg : integer;
    sampleecg, xecgfilter, outlpf, outhpf, outddt, outsqr, th, outp, rpeak : arraybaru;
    fsecg : extended;
    mag2, xecg : vektor;
    datazp : integer;
  end;
...
var
  ...
  hitungRPeak : integer;
  LN, LP, v, vQ : vektor;
  max, tau, an, ap : extended;
begin
      //LANGKAH PERTAMA LPF
    for i := 0 to jmldataecg-1 do
      begin
        outlpf[i] := (2*outlpf[i-1])-outlpf[i-2] + xecgfilter[i] - 2*xecgfilter[i-6] + xecgfilter[i-12];
        lpfofflinechart.AddXY(i,outlpf[i]);
      end;
    //LANGKAH KEDUA HPF
    for i := 0 to jmldataecg-1 do
      begin
        outhpf[i]:= outhpf[i-1]-(outlpf[i]/32)+outlpf[i-16]-outlpf[i-17]+(outlpf[i-32]/32);
        hpfofflinechart.AddXY(i, outhpf[i]);
      end;
    //LANGKAH KETIGA DERIVATIVE
    for i := 0 to jmldataecg-1 do
      begin
        outddt[i]:= (2*outhpf[i] + outhpf[i-1])- outhpf[i-3]-(2*outhpf[i-4]/8);
        derivativeofflinechart.AddXY(i, outddt[i]);
      end;
    //LANGKAH KEEMPAT SQUARE
    for i := 0 to jmldataecg-1 do
      begin
        outsqr[i]:= sqr(outddt[i]);
        sqrofflinechart.AddXY(i, outsqr[i]);
      end;

  lpfofflinechart.Title := 'LPF';
  hpfofflinechart.Title := 'HPF';
  derivativeofflinechart.Title := 'Derivative';
  sqrofflinechart.Title := 'Square';

  //MAV FILTER
  ordemav := strtoint(OrdeMAVEdit.Text);

  for i := 0 to jmldataecg-1 do   //BACKWARD
    begin
      for j:= 0 to ordemav-1 do
       begin
  ...
  for i := 0 to jmldataecg-1 do   //FORWARD
    begin
      for j := 0 to ordemav-1 do
        begin
        ...
        MAVofflineChart.AddXY(i,yforward[i]);
    end;

    MAVofflinechart.Title := 'MAV';
  ...
  for i:=0 to jmldataecg-1 do
   ...
   tau := strtofloat(FFEdit.Text);
   AN := strtofloat(apedit.Text);
   AP := AN;

   for i:=0 to jmldataecg-1 do
    begin
      if yforward[i]>v[i-1] then v[i]:=yforward[i]
      else
        begin
          v[i]:=v[i-1];
          if yforward[i]<=v[i-1]/2 then v[i]:=yforward[i];
        end;
        th[i]:=LN[i-1]+tau*abs(LP[i-1]-LN[i-1]);
        ThOfflineChart.AddXY(i,th[i]);


      if v[i]>th[i] then
        begin
          LP[i]:= ((1-AP)*v[i])+(AP*LP[i-1]);
        end
      else
        begin
          LN[i]:=((1-AN)*v[i])+(AN*LN[i-1]);
        end;
        LPOfflineChart.AddXY(i,LP[i]);
        LNOfflineChart.AddXY(i,LN[i]);
    ...
    for i := 0 to jmldataecg-1 do
      begin
        if (outp[i-1]=1) AND (outp[i] = 0) then
          begin
            rpeakchart.AddXY(i-error, rpeak[i-error]);
          end;
      end;
end;
...
begin
  lpfofflinechart.Clear;
  hpfofflinechart.Clear;
  derivativeofflinechart.Clear;
  sqrofflinechart.Clear;
  MAVofflinechart.Clear;
  rpeakchart.Clear;
  series7.Clear;
  thOfflinechart.Clear;
  LPOfflineChart.Clear;
  LNOfflineChart.Clear;
  listbox1.Clear;
